...
txt(s,'عروض برامج بكالوريوس',85,438,485,44,26,C.navy,true,'center');
...
s=await base(3,content[2].title,true);await phone(s,'home');
label(s,'نسخة ويب محلية  •  عربي + English',850,145,355,true);
...
box(s,475,583,730,65,'#F3EAD7','#DFC98F');txt(s,'المخرج: قرار يناقشه الطالب مع أسرته والمرشد',495,590,690,49,25,C.navy,true,'center');
...
s=await base(6,content[5].title,true);
const sourceCards=[
...
box(s,160,363,1045,104,'#F3EAD7','#DFC98F');txt(s,'مُرشِدي يضيف سياق الطالب',190,374,985,42,31,C.navy,true,'center');txt(s,'الاهتمامات  +  ميزانية الأسرة  +  مقارنة موحّدة  +  مصدر وتاريخ',190,418,985,34,22,C.muted,false,'center');
box(s,160,487,1045,67,'#123D76','#58749B');txt(s,'النتيجة: قرار شخصي قابل للتفسير',190,496,985,49,28,C.white,true,'center');
box(s,160,575,1045,66,'#0A2D61','#58749B');txt(s,'دراستان عشوائيتان  •  أكثر من 77 ألف مشارك  •  ندعم المنطق ونختبر الأثر محليًا',182,583,1001,50,21,C.pale,true,'center');
...
s=await base(9,content[8].title);
const pilot=[['100','طالب',860],['3','مدارس',465],['6','أسابيع',70]];
for(const [value,name,x] of pilot){txt(s,value,x,186,340,102,78,C.navy,true,'center');txt(s,name,x,286,340,52,32,C.gold,true,'center');}
box(s,280,365,925,58,C.navy,C.navy);txt(s,'نقيس قبل الاستخدام وبعده',300,372,885,43,27,C.white,true,'center');
const gates=[['70 طالبًا','يكملون مقارنة موثقة',835],['+1 نقطة','تحسن وضوح ذاتي',455],['مؤسستان','تبدآن تجربة مدفوعة',75]];
for(const [value,name,x] of gates){box(s,x,461,340,126,C.white,'#D4DEE9');txt(s,value,x+15,475,310,55,38,C.navy,true,'center');txt(s,name,x+18,535,304,38,21,C.muted,false,'center');}
txt(s,'أهداف تجربة معلنة، وليست نتائج محققة  •  إذا لم تتحقق نعدّل قبل التوسع',120,615,1085,34,19,C.muted,false,'center');

s=await base(10,content[9].title,true,42);
const stages=[
 {x:855,when:'اليوم',title:'نسخة ويب عاملة',body:'عربي وإنجليزي\nلقطة بيانات موثقة\nوشاشات حقيقية'},
