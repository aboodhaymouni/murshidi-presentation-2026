...
$deck=$null
try {
 $deck=$pptApp.Presentations.Open((Join-Path $outputDir 'Murshidi-Vcoders-Judges-2026-Final-7min-Refined.pptx'),-1,0,0)
 if($deck.Slides.Count -ne 12){throw "Expected 12 slides, found $($deck.Slides.Count)"}
 $deck.SaveAs((Join-Path $outputDir 'Murshidi-Vcoders-Judges-2026-Final-7min-Refined.pdf'),32)
