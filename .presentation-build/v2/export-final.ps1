...
$deck=$null
try {
 $deck=$pptApp.Presentations.Open((Join-Path $outputDir 'Murshidi-Vcoders-Judges-2026-FINAL-v11.pptx'),-1,0,0)
 if($deck.Slides.Count -ne 12){throw "Expected 12 slides, found $($deck.Slides.Count)"}
 $deck.SaveAs((Join-Path $outputDir 'Murshidi-Vcoders-Judges-2026-FINAL-v11.pdf'),32)
 $renderDir=Join-Path $taskBuild 'final-rendered-v11'
