...
& 'C:/Users/Abdalrahman/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node.exe' (Join-Path $taskBuild 'normalize-typography.mjs')
if($LASTEXITCODE -ne 0){throw 'Typography normalization failed'}
Add-Type @'
using System;
...
try {
 ...
 $timings=(Get-Content -Raw -LiteralPath (Join-Path $taskBuild 'content.json') | ConvertFrom-Json | ForEach-Object {$_.seconds})
